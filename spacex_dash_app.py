...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.RangeSlider(id='payload-slider',
                                                 ...
                                                 marks={
                                                     0: {'label': f'{min_payload} kg'},
                                                     2500: {'label': '2500 kg'},
                                                     5000: {'label': '5000 kg'},
                                                     7500: {'label': '7500 kg'},
                                                     10000: {'label': f'{max_payload} kg'}
                                                 }
                                                ),
...
def update_scatter_chart(site, payload_range):
    if site == 'ALL':
        ...
        df = spacex_df[(spacex_df['Launch Site'] == site) & 
                       ...
                       (spacex_df['Payload Mass (kg)'] <= payload_range[1])]
        title = f'Correlation between Payload Mass and Success for Site {site}'

    fig = px.scatter(df, x='Payload Mass (kg)', y='class', color='Booster Version Category',
                     title=title, hover_data=['Launch Site'])
